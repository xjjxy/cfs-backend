...
public class ProjectController {
	...
	@RequestMapping(value = "/addPhase", method = RequestMethod.GET)
	@ResponseBody
	public String addPahse(Phase phase, int project_id) {
		Map<String, Object> result = new HashMap<String, Object>();
		phase.setPhase_time(System.currentTimeMillis());
		System.out.println("phase:" + phase + ",pid:" + project_id);
		try {
			detailDao.addPhase(project_id, phase);
			result.put("type", "1");
	...
	@ResponseBody
	public String getPhase(int project_id) {
		Map<String, Object> result = new HashMap<String, Object>();
		int phaseSize = detailDao.getPhaseSize(project_id);
		List<Phase> list = new ArrayList<Phase>();

		for (int i = 0; i < phaseSize; i++) {
			list.add(detailDao.getPhase(i, project_id));
		}
	...
	@ResponseBody
	public String addProject(Project p, String bTime, String eTime, @RequestPart("showImg") MultipartFile showImg)
			throws Exception {
